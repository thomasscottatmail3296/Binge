...
REQUIRED = [
    "Directory/Directory.md", "01 - TV Shows/TV Shows.md", "02 - Movies/Movies.md",
    "03 - Documentaries/Documentaries.md", "04 - Franchises/Franchises.md",
    "05 - Shorts/Shorts.md", "06 - Collections/Collections.md",
    "07 - Data/Build Manifest.json", "07 - Data/inventory.json",
    "07 - Data/media.schema.json", "07 - Data/inventory.schema.json",
    "08 - Dev Log/Dev Log.md", "09 - Sources/Sources.md", "10 - Templates/Templates.md",
]
...
if isinstance(inventory, dict):
    ...
    if not isinstance(inventory.get("cover_groups"), list):
        ...
        ids = []
        for index, item in enumerate(inventory["media"]):
            if not isinstance(item, dict):
                errors.append(f"inventory.json media[{index}] must be an object")
                continue
            if not item.get("id"):
                errors.append(f"inventory.json media[{index}] is missing id")
            if not item.get("title"):
                errors.append(f"inventory.json media[{index}] is missing title")
            if item.get("id"):
                ids.append(item["id"])
        for duplicate in sorted({x for x in ids if ids.count(x) > 1}):
            errors.append(f"Duplicate inventory media id: {duplicate}")
...
if isinstance(manifest, dict):
    ...
    inventory_media_count = len(inventory.get("media", [])) if isinstance(inventory, dict) and isinstance(inventory.get("media"), list) else 0
    ...
    if manifest.get("media_count") != inventory_media_count:
        errors.append(f"Manifest media_count={manifest.get('media_count')} but inventory contains {inventory_media_count} media entries")
